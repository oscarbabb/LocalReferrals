...
- **Category Description Translations** (October 21, 2025): Implemented complete bilingual support for category descriptions across the platform. Added categoryDescriptionTranslations object to serviceTaxonomy.ts with 55 category ID → Spanish/English description pairs. Created getCategoryDescription helper function in serviceTranslations.ts following the same pattern as getCategoryLabel, with proper type safety (handling null descriptions via || undefined). Updated service-card.tsx and onboarding-tour.tsx to use the translation helper instead of raw database descriptions. E2E tested and architect-approved - category descriptions now translate correctly on services page and in onboarding tour when language is switched, maintaining consistency with existing category label translations.
